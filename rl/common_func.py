...
def convert_h(h_vec, h_pid, h_vec_, h_pid_):
    """
    对入参填充和定义mask
    """
    ...
    max_len_ = max([len(i) for i in h_pid_])
    h_mask_ = [[1 for _ in i] for i in h_pid_]
    h_vec_ = pad_2d(h_vec_, max_len_, pad_id=[0, 0, 0, 0, 0])
    h_mask_ = pad_2d(h_mask_, max_len_, pad_id=0)
    h_pid_ = pad_2d(h_pid_, max_len_, pad_id=0)

    return h_vec, h_pid, h_mask, h_vec_, h_pid_, h_mask_
