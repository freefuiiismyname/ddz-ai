...
# AI模型类
class RL(object):
    def __init__(self, s_dim, lr_a=0.0001, lr_c=0.0002, reward_decay=0.95, memory_size=360, batch_size=36, dim_size=5,
        ...
        self.act_ids_ = tf.placeholder(tf.int32, [None, None, None], 'a_s_')
        self.act_nd_ids_ = tf.placeholder(tf.int32, [None, None, None, 2], 'a_s_nd_')
        self.d_v_ = tf.placeholder(tf.float32, [None, None, self.dim_size], 'd_v_')
        self.a_m_ = tf.placeholder(tf.float32, [None, None, None], 'a_m_')
        self.l_m_ = tf.placeholder(tf.int32, [None, None, None], 'l_m_')
        self.h_vec_ = tf.placeholder(tf.float32, [None, None, self.dim_size], 'h_v_')
        self.h_pid_ = tf.placeholder(tf.int32, [None, None], 'h_p_')
        self.h_m_ = tf.placeholder(tf.float32, [None, None], 'h_m_')
        """
        yield model
        """
        ...
        with tf.variable_scope('F'):
            ...
            with tf.variable_scope('Critic'):
                ...
                with tf.variable_scope('target'):
                    act_vec_ = tf.gather_nd(self.d_v_, self.act_nd_ids_)
                    q_ = self.build_c(act_vec_, self.S_, self.a_m_, self.l_m_, self.h_vec_, self.h_pid_, self.h_m_)
        ...
        self.ct_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='F/Critic/target')

        td_error = self.R + self.reward_decay * q_ - q
        self.c_loss = tf.reduce_mean(td_error ** 2)
        ...
        self.a_loss = -tf.reduce_mean(tf.log(self.acts_prob) * td_error)
    ...
    def learn_f(self):
        ...
        state_, act_ids_, label_masks_, attn_masks_, dyn_vec_ = [], [], [], [], []
        h_vec, h_pid, h_vec_, h_pid_ = [], [], [], []

        for i, idx in enumerate(indices):
            h_vec.append(self.f_memory['h_vec'][idx])
            h_pid.append(self.f_memory['h_pid'][idx])
            h_vec_.append(self.f_memory['h_vec_'][idx])
            h_pid_.append(self.f_memory['h_pid_'][idx])
            ...
            state_.append(self.f_memory['s_'][idx])
            act_ids_.append(self.f_memory['a_s_'][idx])
            label_masks_.append(self.f_memory['l_m_'][idx])
            attn_masks_.append(self.f_memory['a_m_'][idx])
            dyn_vec_.append(self.f_memory['d_v_'][idx])
        act_ids, label_masks, attn_masks, dyn_vec = convert_alad(act_ids, label_masks, attn_masks, dyn_vec)
        act_ids_, label_masks_, attn_masks_, dyn_vec_ = convert_alad(act_ids_, label_masks_, attn_masks_, dyn_vec_)

        h_vec, h_pid, h_m, h_vec_, h_pid_, h_m_ = convert_h(h_vec, h_pid, h_vec_, h_pid_)
        # [batch_size, combo_nums, combo_length, 2]
        act_nd_ids = [[[[idx, k] for k in j] for j in i] for idx, i in enumerate(act_ids)]
        act_nd_ids_ = [[[[idx, k] for k in j] for j in i] for idx, i in enumerate(act_ids_)]
        ...
        _, _, a_loss, c_loss = self.sess.run([self.a_train, self.c_train, self.a_loss, self.c_loss],
                                             {self.S: state, self.l_m: label_masks,
                                              ...
                                              self.f_action: action,
                                              self.l_m_: label_masks_, self.a_m_: attn_masks_,
                                              self.act_ids_: act_ids_, self.h_vec_: h_vec_, self.h_pid_: h_pid_,
                                              self.act_nd_ids_: act_nd_ids_, self.h_m_: h_m_,
                                              self.d_v_: dyn_vec_, self.S_: state_})
        return [a_loss, c_loss]

    def store_f(self, _state, _act_ids, _label_masks, _attn_masks, _dyn_vec, _action, _h_vec, _h_pid, reward,
                state, act_ids, label_masks, attn_masks, dyn_vec, h_vec, h_pid):
        """
        存储斗地主记忆
        """
        if 's' not in self.f_memory:
            for name in ['s', 'a_s', 'd_v', 'l_m', 'a_m', 'h_pid', 'h_vec', 'a', 'r', 's_', 'a_s_', 'd_v_', 'a_m_',
                         'l_m_', 'h_pid_', 'h_vec_']:
        ...
        self.f_memory['s_'][index] = state
        self.f_memory['a_s_'][index] = act_ids
        self.f_memory['d_v_'][index] = dyn_vec
        self.f_memory['l_m_'][index] = label_masks
        self.f_memory['a_m_'][index] = attn_masks
        self.f_memory['h_vec_'][index] = h_vec
        self.f_memory['h_pid_'][index] = h_pid
        self.f_pointer += 1
